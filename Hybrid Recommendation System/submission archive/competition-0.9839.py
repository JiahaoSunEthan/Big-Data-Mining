...
def extract_business_feature(row):
    latitude = row['latitude']  # is None
    longitude = row['longitude']
    stars = row['stars']
    review_count = row['review_count']
    category = row['categories'].split(',') if row['categories'] is not None else []
    return (row['business_id'],(latitude,longitude,stars,review_count,category))
...
raw_user_info_map = raw_user_info.collectAsMap()
...
avg_business_info = [business_info.map(lambda x:x[1][i]).mean() for i in range(2,4)]
...
def generate_feature_train(row):
    ...
    features = []
    '''
    if user_id in user_avg_map:
        avg_rating_u = user_avg_map[user_id]
    else:
        avg_rating_u = user_avg_rating
    if business_id in business_avg_map:
        avg_rating_b = business_avg_map[business_id]
    else:
        avg_rating_b = business_avg_rating
    '''
    ...
    if business_id in business_info_map:
        bus_feat = list(business_info_map[business_id])[2:4]
    else:
