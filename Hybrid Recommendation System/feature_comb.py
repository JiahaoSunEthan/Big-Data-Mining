# -*- coding: utf-8 -*-
"""
Created on Sun Dec  5 19:06:52 2021
...
def compute_rating_train(row,n):
    user = row[0]
    business = row[1]
    if business not in business_map:
        if user not in user_map: #### new business & new user
            ...
            return [user,business,user_avg_map[user]]
    else:
        if user not in user_map: ### old business & new user
            return [user,business,business_avg_map[business]]
        else:  ### old business & old user
            neighbour = []
            for bus_neigh in user_map[user].keys():
                if bus_neigh!=business and compute_pearson_correlation(business,bus_neigh)> 0: #only consider pearson coef >0
            ...
            if len(neighbour) ==0:
                ...
                return [user,business,default_rating]
            ...
            return [user,business,rating_numerator/rating_denominator]
...
train_predict = train.map(lambda row:compute_rating_train(row,8)).map(cutscore).collectAsMap()
test_predict = test.map(lambda row:compute_rating(row,8)).map(cutscore).collectAsMap() 
...
def generate_feature_train(row):
    ...
    features.append(train_predict[(user_id,business_id)])
...
def generate_feature_test(row):
    ...
    features.append(checkin_num)
    features.append(photo_num)
    features.append(test_predict[(user_id,business_id)])
    return features


train_df = pd.DataFrame(train.map(lambda row:generate_feature_train(row)).collect(),columns = ['feat1','feat2','feat3','feat4','feat5','feat6','feat7','feat8','feat9','feat10','feat11','feat12','feat13','feat14','ratings'])
test_df = pd.DataFrame(test.map(lambda row:generate_feature_test(row)).collect(),columns = ['user_id','business_id','feat1','feat2','feat3','feat4','feat5','feat6','feat7','feat8','feat9','feat10','feat11','feat12','feat13','feat14'])
