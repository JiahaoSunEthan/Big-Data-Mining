# -*- coding: utf-8 -*-
"""
Created on Fri Dec  3 23:26:08 2021
...
boost_res_rdd = spark.createDataFrame(boost_res).rdd.map(cutscore_v2)
#test_boost = spark.createDataFrame(test_predict_m).rdd.map(cutscore_v2)
whole_rdd = train.union(boost_res_rdd)
#final_rdd = whole_rdd.union(test_boost)
...
y_test_pred3 = regr.predict(x_test)
test_predict_m3 = pd.concat([test_df.iloc[:,[0,1]],pd.DataFrame(y_test_pred3)],axis=1)
test_predict_m3.columns=['user_id','business_id','prediction_1']
def cutscore_df(x):
    if x['prediction_1']>5:
        return 5.0
    elif x['prediction_1']<1:
        ...
        return x['prediction_1']
test_predict_m3.loc[:,'prediction'] = test_predict_m3.apply(cutscore_df,axis = 1)
test_predict_m3 = test_predict_m3.drop(labels = ['prediction_1'],axis = 1)

prediction_1 = pd.merge(test_predict_cf,test_predict_m,on =['user_id','business_id'])
prediction = pd.merge(prediction_1,test_predict_m2,on =['user_id','business_id'])
prediction.columns=['user_id','business_id','prediction_x','weight','prediction_y','prediction_z']
def final_prediction(x):
    flag1 = abs(x['prediction_x']-round(x['prediction_x']))<=0.1
    flag2 = abs(x['prediction_y']-round(x['prediction_y']))<=0.2
    flag3 = abs(x['prediction_z']-round(x['prediction_z']))<=0.1
    flag = round(x['prediction_x'])==round(x['prediction_y']) and round(x['prediction_z'])==round(x['prediction_y'])
    if flag ==True and flag1+flag2+flag3 >=2:
        final = round(x['prediction_y'])
    else:
        #final = x['weight']/10*x['prediction_x']+(1-x['weight']/10)*x['prediction_y']
        final = x['prediction_y']
...
RMSE =  pd.merge(test_data,test_predict_m3,on =['user_id','business_id'])
def mse(x):
